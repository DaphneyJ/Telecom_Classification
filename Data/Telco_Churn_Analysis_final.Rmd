...
gbm_auc <- roc(as.numeric(test_gbm$Churn), gbm_pred_prob[,1])
...
```{r}
step_pred_prob_train <- predict(step_model, newdata = train, type = "response")
step_pred_train <- ifelse(step_pred_prob_train > 0.5, "Yes", "No")
step_roc_train <- roc(y_train, as.vector(step_pred_prob_train))
lasso_pred_prob_train <- predict(lasso_model, newx = x_train, type = "response")
lasso_roc_train <- roc(y_train, as.vector(lasso_pred_prob_train))
lasso_pred_train <- ifelse(lasso_pred_prob_train > 0.5, 1, 0)
rf_pred_train <- predict(rf_tuned, newdata = train)


gbm_pred_train <- predict(gbm_model, newdata = train_gbm)
gbm_pred_prob_train <- predict(gbm_model, newdata = train_gbm, type = "prob")

xgb_pred_prob_train <- predict(xgb_model, newdata = as.matrix(x_train), type = "prob")[,2]
xgb_pred_train <- ifelse(xgb_pred_prob_train > 0.5, 1, 0)

rf_auc_train <- roc(train$Churn, as.numeric(rf_pred_train == "Yes"))
gbm_auc_train <- roc(as.numeric(test_gbm$Churn), gbm_pred_prob[,1])
xgb_auc_train <- roc(y_train, xgb_pred_train)

results_all_train <- data.frame(
  Model = c("Stepwise Logistic", "LASSO Logistic", "Random Forest", "GBM", "XGBoost"),
  Accuracy = c(
    mean(step_pred_train == train$Churn),
    mean(lasso_pred_train == y_train),
    mean((as.numeric(rf_pred_train)-1) == y_train),
    mean(gbm_pred_train == train_gbm$Churn),
    mean(xgb_pred_train == y_train)
  ),
  AUC = c(
    auc(step_roc_train),
    auc(lasso_roc_train),
...
print(results_all_train)
```

```{r}
xg_param<-as.list(xgb_model$bestTune)
 xg_rounds=xgb_model$bestTune$nrounds
#  xg_depth=xgb_model$bestTune$max_depth
#  xg_gamma=xgb_model$bestTune$gamma
 # xg_eta=xgb_model$bestTune$eta
  #xg_sample=xgb_model$bestTune$colsample_bytree
  #xg_weight=xgb_model$bestTune$min_child_weight
  #xg_sub=xgb_model$bestTune$subsample
rf_try=rf_tuned$finalModel$tuneValue$mtry
gbm_ntrees=gbm_model$bestTune$n.trees
gb_depth=gbm_model$bestTune$interaction.depth
gbm_shrinkage=gbm_model$bestTune$shrinkage
gbm_obs=gbm_model$bestTune$n.minobsinnode
```
...
for (b in 1:B){
  ...
  rf_model_cv <- randomForest(Churn ~ ., data = train, ntree = 100,mtry=rf_try) 
  rf_pred_prob <- predict(rf_model_cv, newdata = test, type = "prob")[, "Yes"] 
  rf_pred <- predict(rf_model_cv, newdata = test) 
  ...
  gbm_model_cv <- gbm(Churn ~ ., data = train_gbm, distribution = "bernoulli",
                 n.trees = gbm_ntrees, shrinkage = gbm_shrinkage, interaction.depth = gb_depth,n.minobsinnode=gbm_obs)

  gbm_pred_prob <- predict(gbm_model_cv, newdata = test_gbm, n.trees = gbm_ntrees, type = "response")
  ...
  xgb_train <- xgb.DMatrix(data = x_train, label = y_train)
  xgb_test <- xgb.DMatrix(data = x_test, label = y_test)
  xgb_model_cv <- xgboost(data = xgb_train, params=xg_param, objective = "binary:logistic", nrounds = xg_rounds, verbose = 0)
  xgb_pred_prob <- predict(xgb_model_cv, newdata = xgb_test)
  xgb_pred <- ifelse(xgb_pred_prob > 0.5, 1, 0)
...
#Random Forest ROC, LASSO recall fail by a lot, stepwise recalle, random forest recall, and GBM recall are the ones that aren't normal, all but are close
...
T1_roc=t.test(monte_df[,5],monte_df[,1],paired=T)
T2_roc=t.test(monte_df[,5],monte_df[,2],paired=T)
T3_roc=t.test(monte_df[,5],monte_df[,3],paired=T)
T4_roc=t.test(monte_df[,5],monte_df[,4],paired=T)
...
T1_recall=t.test(monte_df[,19],monte_df[,16],paired=T)
T2_recall=t.test(monte_df[,19],monte_df[,17],paired=T)
T3_recall=t.test(monte_df[,19],monte_df[,18],paired=T)
T4_recall=t.test(monte_df[,19],monte_df[,20],paired=T)
...
comp_names_xg<-c( "XGBoost = LASSO?","XGBoost = Stepwise?","XGBoost = Random Forest?" ,"XGBoost = Naive GBM?")
...
t_table_xg<-data.frame(
  Hypothesis=comp_names_xg,
