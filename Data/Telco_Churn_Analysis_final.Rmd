...
print("Revenue lost per month from a lost customer = Lifetime value / Tenure in Months")
cat(" Revenue lost per month from churned customers: $",toString(round(average_monthly_churn,2)))
```
...
```{r}
ggplot(melt(km$centers), aes(x=Var1, y=value)) + 
  geom_bar(stat = "identity") +
facet_wrap(~Var2,scale="free") +
  ggtitle("Average score by variable, by cluster (scaled)")
...
shapiro.test(monte_df[,2])$p.value,
shapiro.test(monte_df[,3])$p.value,
shapiro.test(monte_df[,4])$p.value,
shapiro.test(monte_df[,5])$p.value
)
...
t_scores<-c(T1_roc$p.value,T2_roc$p.value,T3_roc$p.value,T4_roc$p.value)
comp_names_roc<-c( "Vs LASSO","Vs Random Forest","Vs Naive GBM","Vs XGBoost")
t_table_roc_df<-data.frame(
  Model=comp_names_roc,
  p_value_t_test_roc=t_scores
)
t_table_roc_df
